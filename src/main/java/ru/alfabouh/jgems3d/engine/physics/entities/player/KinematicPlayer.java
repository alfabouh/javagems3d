...
import ru.alfabouh.jgems3d.engine.physics.entities.IBtEntity;
...
public class KinematicPlayer extends WorldItem implements IPlayer, IInventoryOwner, IWorldTicked, IBtEntity {
    ...
    public KinematicPlayer(World world, @NotNull Vector3f pos, @NotNull Vector3f rot) {
    ...
    public void performController(Vector2f rotationInput, Vector3f xyzInput, boolean isFocused) {
        if (JGems.DEBUG_MODE) {
            if (JGemsControllerDispatcher.bindingManager().keyBlock1.isClicked()) {
                BtDynamicMeshBody entityPropInfo = new BtDynamicMeshBody(JGemsResourceManager.modelAssets.cube, this.getWorld(), this.getPosition().add(this.getLookVector().mul(5.0f)), "test");
                JGems.get().getProxy().addItemInWorlds(entityPropInfo, JGemsResourceManager.renderDataAssets.entityCube);

                PointLight pointLight = new PointLight().setBrightness(10.0f);
