...
public class EntitySodaItem extends WorldItem {
    private PickUpItemTriggerZone pickUpItemTriggerZone;

    public EntitySodaItem(World world, Vector3f pos, String itemName) {
                ...
                (e) -> {
                    if (e instanceof KinematicPlayerSP) {
                        ...
                        if (!kinematicPlayerSP.isHasSoda()) {
                            kinematicPlayerSP.setHasSoda(true);
                            JGems.get().getSoundManager().playLocalSound(JGemsResourceManager.soundAssetsLoader.pick, SoundType.BACKGROUND_SOUND, 1.0f, 1.0f);
                            SystemLogging.get().getLogManager().log("Put soda in inventory!");
                            this.setDead();
                        }
                    }
                });
