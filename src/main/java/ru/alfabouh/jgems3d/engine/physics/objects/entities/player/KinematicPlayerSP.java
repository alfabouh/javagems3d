...
public class KinematicPlayerSP extends WorldItem implements IPlayer, JBulletEntity, IWorldDynamic, IHasInventory {
    ...
    private final Vector3f walking;
    ...
    private final Vector3f currentHitScanCoordinate;
    ...
    public KinematicPlayerSP(World world, @NotNull Vector3f pos, @NotNull Vector3f rot) {
        super(world, pos, rot, "player_sp");
        this.cameraRotation = new Vector3f();
        this.inputMotion = new ArrayDeque<>();
        this.walking = new Vector3f();
        this.currentHitScanCoordinate = new Vector3f(0.0f);
    ...
    public Vector3f getCollisionTranslation() {
    ...
    public void setCollisionTranslation(Vector3f pos) {
        try (btTransform transform = this.getBulletObject().getWorldTransform()) {
    ...
    public void setCollisionRotation(Vector3f angles) {
        try (btTransform transform = this.getBulletObject().getWorldTransform()) {
    ...
    private btVector3 getWalkingSpeed(Vector3f motion) {
        ...
        if (new Vector3f(motion.x, 0, motion.z).length() > 0) {
            if (this.kinematicCharacterController.onGround() && this.getTicksExisted() % (this.isRunning ? 15 : 20) == 0) {
                this.playStepSound();
            }
            this.walking.set(new Vector3f(motion).mul(1, 0, 1));
    ...
    public void onUpdate(IWorld iWorld) {
        ...
        Vector3f look = MathHelper.calcLookVector(this.getRotation());
        this.hitScan(look);
        ...
        if (this.getPosition().y <= -50 || this.getPosition().y >= 500) {
            this.setCollisionTranslation(new Vector3f(0, 5, 0));
        }
        Vector3f cVector = this.calcControllerMotion();
    ...
    public Vector3f getCurrentHitScanCoordinate() {
        return new Vector3f(this.currentHitScanCoordinate);
    }

    private void hitScan(Vector3f look) {
    ...
    private void grabTest(Vector3f look) {
        this.wantsToSelect = null;
    ...
    public void performController(Vector2f rotationInput, Vector3f xyzInput, boolean isFocused) {
        ...
        if (this.currentController() instanceof MouseKeyboardController) {
            ...
            if (mouseKeyboardController.getMouseAndKeyboard().isRightKeyPressed()) {
                ...
                //System.out.println("news Vector3f(" + x + ", " + y + ", " + z + ")");
        ...
        if (this.inputMotion.size() < PhysicThreadManager.TICKS_PER_SECOND * 10) {
            ...
            if (new Vector3f(xyzInput).mul(1, 0, 1).length() > 0 && xyzInput.y < 0) {
    ...
    public float getEyeHeight() {
        return (float) (0.45f + Math.sin(RenderPlayerSP.stepBobbing * 0.2f) * 0.1f);
    }
