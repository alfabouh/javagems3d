...
public class LightManager implements ILightManager {
    ...
    private void updateSunUbo(Matrix4f viewMatrix) {
        ...
        value1Buffer.flip();
        JGemsSceneRender.getGameUboShader().performUniformBuffer(ResourceManager.shaderAssets.SunLight, value1Buffer);
    ...
    private void updatePointLightsUbo() {
        ...
        for (int i = 0; i < total; i++) {
            ...
            value1Buffer.flip();
            JGemsSceneRender.getGameUboShader().performUniformBuffer(ResourceManager.shaderAssets.PointLights, i * (8 * 4), value1Buffer);
        }
        MemoryUtil.memFree(value1Buffer);
        ...
        intBuffer.put(total);
        intBuffer.flip();
        JGemsSceneRender.getGameUboShader().performUniformBuffer(ResourceManager.shaderAssets.PointLights, LightManager.MAX_POINT_LIGHTS * (8 * 4), intBuffer);
    ...
    public void removeAllLights() {
        ...
        for (int i = 0; i < this.getPointLightList().size(); i++) {
            ...
            value1Buffer.put(-1);
            value1Buffer.flip();
            JGemsSceneRender.getGameUboShader().performUniformBuffer(ResourceManager.shaderAssets.PointLights, i * 32, value1Buffer);
        }
        MemoryUtil.memFree(value1Buffer);
    }
}