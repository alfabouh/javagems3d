...
import ru.alfabouh.jgems3d.engine.graphics.opengl.rendering.programs.fbo.FBOTexture2DProgram;
...
public class MainMenuPanel extends AbstractPanelUI {
    ...
    public void drawPanel(ImmediateUI immediateUI, float partialTicks) {
        ...
        immediateUI.buttonUI("Map TBox", JGemsResourceManager.textureAssets.buttonFont, new Vector2i(windowW / 2 - 150, windowH / 2 - 210), new Vector2i(300, 60), 0xffffff, 0.5f)
                .setOnClick(() -> {
                    JGems.get().loadMap("default_map");
                });

        immediateUI.buttonUI("Sponza", JGemsResourceManager.textureAssets.buttonFont, new Vector2i(windowW / 2 - 150, windowH / 2 - 120), new Vector2i(300, 60), 0xffffff, 0.5f)
                .setOnClick(() -> {
        ...
        immediateUI.buttonUI("Map02", JGemsResourceManager.textureAssets.buttonFont, new Vector2i(windowW / 2 - 150, windowH / 2 - 30), new Vector2i(300, 60), 0xffffff, 0.5f)
                .setOnClick(() -> {
                    JGems.get().loadMap(new Map02());
                });
    ...
    private void renderContent(ImmediateUI immediateUI, Window window, float partialTicks) {
        ...
        immediateUI.textUI(JGems.get().toString(), JGemsResourceManager.textureAssets.standardFont, new Vector2i(10, windowH - 35), 0x00ff00, 0.5f);

        if (MainMenuPanel.showBlood) {
            int seconds = JGems.get().getPhysicsWorld().getTicks() / PhysicsThread.TICKS_PER_SECOND;
            String time = (seconds / 60) + "min, " + (seconds % 60) + "sec";
            String gOverText = JGems.get().I18n("menu.main.gameplay.dead", time);

            int tWidth = ImmediateUI.getTextWidth(JGemsResourceManager.textureAssets.standardFont, gOverText);
            int tHeight = ImmediateUI.getFontHeight(JGemsResourceManager.textureAssets.standardFont);
            immediateUI.textUI(gOverText, JGemsResourceManager.textureAssets.standardFont, new Vector2i(windowW / 2 - tWidth / 2, windowH / 2 - tHeight / 2 - 100), 0xff0000, 0.5f);
            ...
            int tWidth = ImmediateUI.getTextWidth(JGemsResourceManager.textureAssets.standardFont, gOverText);
            int tHeight = ImmediateUI.getFontHeight(JGemsResourceManager.textureAssets.standardFont);
            immediateUI.textUI(gOverText, JGemsResourceManager.textureAssets.standardFont, new Vector2i(windowW / 2 - tWidth / 2, windowH / 2 - tHeight / 2 - 100), 0x00ff00, 0.5f);
        }
    }
