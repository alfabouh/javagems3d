package ru.alfabouh.jgems3d.engine.graphics.opengl.rendering.imgui.elements;

import org.jetbrains.annotations.NotNull;
import org.joml.Vector2f;
import org.joml.Vector2i;
import ru.alfabouh.jgems3d.engine.math.MathHelper;
import ru.alfabouh.jgems3d.engine.graphics.opengl.rendering.imgui.elements.base.UIInteractiveElement;
import ru.alfabouh.jgems3d.engine.graphics.opengl.rendering.imgui.elements.base.UIScalable;
...
public class UISlider extends UIInteractiveElement implements UIScalable {
    ...
    public void render(float partialTicks) {
        ...
        UIPictureStatic line = new UIPictureStatic(JGemsResourceManager.textureAssets.gui1, new Vector2i(this.getPosition()).add(0, (int) (3.0f * this.getScaling().y)), new Vector2f(0.0f, 13.0f), new Vector2f(100.0f, 1.0f), this.getZValue());
        UIPictureStaticSelectable brick = new UIPictureStaticSelectable(JGemsResourceManager.textureAssets.gui1, new Vector2i(this.getPosition()).add((int) (this.getOptionValue() * this.getScaling().mul(98).x), 0), new Vector2f(0.0f, 14.0f), new Vector2f(2.0f, 7.0f), this.getZValue());
    ...
    protected void onClicked(Vector2f mouseCoordinates) {
        float value = MathHelper.clamp((mouseCoordinates.x - this.getPosition().x) / this.getSize().x, 0.0f, 1.0f);
