...
public class LeaveConfirmationPanel extends AbstractPanelUI {
    ...
    public void drawPanel(ImmediateUI immediateUI, float partialTicks) {
        ...
        int textSize = ImmediateUI.getTextWidth(JGemsResourceManager.textureAssets.standardFont, text);
        immediateUI.textUI(text, JGemsResourceManager.textureAssets.standardFont, new Vector2i(windowW / 2 - textSize / 2, windowH / 2 - 60), 0xffffff, 0.5f);

        immediateUI.buttonUI(JGems.get().I18n("menu.confirm.yes"), JGemsResourceManager.textureAssets.standardFont, new Vector2i(windowW / 2 + 5, windowH / 2), new Vector2i(200, 60), 0xffffff, 0.5f)
                .setOnClick(() -> {
                    JGems.get().destroyGame();
                });
        immediateUI.buttonUI(JGems.get().I18n("menu.confirm.no"), JGemsResourceManager.textureAssets.standardFont, new Vector2i(windowW / 2 - 205, windowH / 2), new Vector2i(200, 60), 0xffffff, 0.5f)
                .setOnClick(() -> {
