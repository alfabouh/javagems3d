package ru.alfabouh.jgems3d.toolbox.resources.utils;
                    ...
                    if (scene != null) {
                        ...
                        for (int i = 0; i < totalMaterials; i++) {
                            try (AIMaterial aiMaterial = AIMaterial.create(Objects.requireNonNull(scene.mMaterials()).get(i))) {
                                materialList.add(SimpleModelLoader.readMaterial(aiMaterial, JGems.normalizeURL((Paths.get(modelPath)).getParent().toString()) + "/"));
                            }
                        }
