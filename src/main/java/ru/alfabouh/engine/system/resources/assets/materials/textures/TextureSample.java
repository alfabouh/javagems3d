...
public class TextureSample implements IImageSample {
    ...
    private TextureSample(boolean inJar, String fullPath, boolean interpolate, int wrapping) {
        ...
        JGems.get().getLogManager().log("Loading " + this.getName());
        if (inJar) {
    ...
    private TextureSample(String id, InputStream inputStream, boolean interpolate, int wrapping) {
        ...
        this.enableAnisotropic = true;
        if (inputStream == null) {
            JGems.get().getLogManager().warn("Error, while loading texture " + id + " InputStream is NULL");
    ...
    private ByteBuffer readTextureFromMemory(String name, InputStream inputStream) {
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            if (imageBuffer == null) {
                JGems.get().getLogManager().warn("Couldn't create texture " + name);
                JGems.get().getLogManager().bigWarn(STBImage.stbi_failure_reason());
            } else {
    ...
    private ByteBuffer readTextureOutsideJar(String path) {
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            if (imageBuffer == null) {
                JGems.get().getLogManager().warn("Couldn't create texture " + path);
                JGems.get().getLogManager().bigWarn(STBImage.stbi_failure_reason());
            } else {
                this.width = width.get();
                this.height = height.get();
    ...
    private void createTexture() {
        ...
        JGems.get().getLogManager().log("Texture " + this.getName() + " successfully created!");
    }
