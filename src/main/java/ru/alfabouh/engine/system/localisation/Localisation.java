...
public class Localisation {
    ...
    public void setCurrentLang(Lang currentlang) {
        if (currentlang == null) {
            JGems.get().getLogManager().warn("Tried to set NULL language");
            currentlang = Lang.English;
        }
        JGems.get().getLogManager().log("Loading language table: " + currentlang.getFullName());
        this.readLangFilesInTable(currentlang);
        JGems.get().getLogManager().log("Language table loaded");
    ...
    private void readStream(LangMap langMap, String fileName) throws IOException {
        try (InputStream inputStream = JGems.loadFileJarSilently("/assets/lang/" + fileName)) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (strings.length == 2) {
                    ...
                    JGems.get().getLogManager().warn("Error in lang file " + fileName + " on line: " + l);
                }
            }
            reader.close();
