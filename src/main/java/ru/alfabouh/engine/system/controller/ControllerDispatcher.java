package ru.alfabouh.engine.system.controller;

import org.joml.Vector2d;
import org.joml.Vector3d;
import ru.alfabouh.engine.JGems;
import ru.alfabouh.engine.system.controller.binding.BindingList;
...
public class ControllerDispatcher {
    ...
    public void attachControllerTo(IController controller, IControllable remoteController) {
        JGems.get().getLogManager().log("Attached controller to: " + ((WorldItem) remoteController).getItemName());
    ...
    public void detachController() {
        if (this.getCurrentControlledItem() != null) {
            JGems.get().getLogManager().log("Detached Controller From: " + ((WorldItem) this.getCurrentControlledItem()).getItemName());
