...
public class TBoxMapReader {
    ...
    public static SaveContainer readMapFolderFromJAR(String mapName) throws IOException, ClassNotFoundException {
        ...
        HashSet<SaveObject> saveObjectSet = null;
        try {
            mapProperties = SerializeHelper.readFromJSON(JGems.loadFileJar(new JGPath("/assets/jgems/maps/" + mapName + "/map_prop.json")), MapProperties.class); //TODO
        } catch (Exception e) {
            e.printStackTrace(System.err);
        }
        try {
            saveObjectSet = SerializeHelper.readFromBytes(JGems.loadFileJar(new JGPath("/assets/jgems/maps/" + mapName + "/objects.ser")), HashSet.class);
        } catch (Exception e) {
            e.printStackTrace(System.err);
        }
        SaveContainer saveContainer = new SaveContainer(mapProperties);
