...
void main()
{
    ...
    vec2 texel_size = 1.0 / textureSize(sampler2D(texture_map), 0);
    ...
    for (int y = -radius; y <= radius; ++y)
    {
        for (int x = -radius; x <= radius; ++x)
        {
            ...
            color += texture(sampler2D(texture_map), uv_coordinates + offset);
        }
    }
...
void main()
{
    ...
    for (int y = -radius; y <= radius; ++y)
    {
        for (int x = -radius; x <= radius; ++x)
        {
            ...
            {
                color += texture(sampler2D(texture_map), sample_uv);
                num_samples++;
            }
        }
    }
...
void main()
{
    ...
    for (int y = -radius; y <= radius; ++y)
    {
        ...
        {
            vec2 offset = vec2(float(x), float(y)) * texel_size;
            color += texture(sampler2D(texture_map), uv + offset);
        }
    }
