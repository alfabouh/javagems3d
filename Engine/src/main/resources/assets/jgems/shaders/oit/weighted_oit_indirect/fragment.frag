...
vec4 calc_light(vec3 frag_pos, vec3 normal, vec4 specularFactor) {
    ...
    for (int i = 0; i < total_plights; i++) {
        ...
        point_light_factor += calc_point_light(p, frag_pos, normal, at_base, linear, expo, p_brightness, specularFactor) * shadow;
    }
...
vec4 calc_point_light(PointLight light, vec3 vPos, vec3 vNormal, float at_base, float linear, float expo, float bright, vec4 specularFactor) {
    vec3 pos = light.plViewPos.xyz;

    vec3 light_dir = pos - vPos;
    vec3 to_light = normalize(light_dir);
    vec4 light_c = calc_light_factor(light.plColor.xyz, bright, vPos, to_light, vNormal, specularFactor);
