...
vec3 calc_normal_map() {
    vec3 normal = texture(sampler2D(normals_map_bindless), getScaledTexture()).rgb;
...
void main()
{
    ...
    if (useDiffuseTexture) {
        diffuse *= texture(sampler2D(diffuse_map_bindless), getScaledTexture());
    }
    ...
    if (useRoughnessMetallicTexture) {
        vec4 mr = texture(sampler2D(metallic_roughness_map_bindless), getScaledTexture());
    ...
    vec4 lights = calc_light(gPosition, gNormal, gMetallicRoughness.g);
