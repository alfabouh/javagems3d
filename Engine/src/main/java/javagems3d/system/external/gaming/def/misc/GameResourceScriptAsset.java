...
public record GameResourceScriptAsset(String name, String relativePath, String scriptText) implements IAsset {
    public void save(@NotNull JGemsPath absoluteGamePath, @Nullable String scriptText) {
        {
            final JGemsPath absPath = new JGemsPath(JGemsGaming.getScriptsFolder(absoluteGamePath), this.relativePath());
            try (FileOutputStream fos = new FileOutputStream(absPath.toFile())) {
                fos.write((scriptText == null ? this.scriptText : scriptText).getBytes(StandardCharsets.UTF_8));
