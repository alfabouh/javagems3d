...
public class CubeMapTextureOLD implements ISample, ICached {
    private int textureId;

    public CubeMapTextureOLD() {
    }

    public static CubeMapTextureOLD createCubeMap(ResourceCache resourceCache, CubeMapTexturingDataPack cubeMapTexturingDataPack) {
        CubeMapTextureOLD cubeMap = new CubeMapTextureOLD();
        cubeMap.registerCubeMap(cubeMapTexturingDataPack);
        if (resourceCache != null) {
            if (resourceCache.checkObjectInCache(cubeMapTexturingDataPack.getName())) {
                return (CubeMapTextureOLD) resourceCache.getCachedObject(cubeMapTexturingDataPack.getName());
            }
            if (cubeMap.isValid()) {
