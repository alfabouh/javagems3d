...
public class InventoryBase {
    ...
    public InventoryBase(@NotNull InventoryOwner owner, int maxSlots) {
    ...
    public boolean consumeItem(Inventory.Item item) {
        if (item == null) {
            throw new JGemsNullException("NULL item removed from inventory");
        }
        for (int i = 0; i < this.getMaxSlots(); i++) {
            if (this.hasItemInSlot(i) && this.getItemInSlot(i).equals(item)) {
                this.consumeItem(i);
                return true;
            }
        }
    ...
    public Inventory.Item getItemInSlot(int slot) {
        return this.getInventorySlots().get(slot).getInventoryItem();
    }

    public void setSlotItem(int slot, Inventory.Item item) {
        if (item != null) {
            item.onAddInInventory(this.getOwner());
        }
        this.getInventorySlots().get(slot).setInventoryItem(item);
        this.checkCurrItem();
    }
    ...
    public Inventory.Item getCurrentItem() {
    ...
    public static class Slot {
        ...
        public void setInventoryItem(Inventory.Item item) {
            this.item = item;
        }
    }
}
