...
public abstract class JGemsKinematicItem extends WorldItem implements IWorldTicked, IBtEntity {
    ...
    protected void createGhostObject() {
        this.physicsGhostObject = new PhysicsGhostObject(this.createShape());
        this.setCollisionGroup(CollisionFilter.PLAYER);
        this.setCollisionFilter(CollisionFilter.ALL);
        this.physicsGhostObject.setUserObject(this);
    }
    ...
    private boolean checkIfOnGround(PhysicsGhostObject physicsGhostObject) {
        SweepResult sweepResult = SweepResult.getSweepHitResult(this.getWorld().getDynamics(), physicsGhostObject, (ConvexShape) this.getPhysicsGhostObject().getCollisionShape(), this.getPosition().add(0.0f, 0.01f, 0.0f), new Vector3f(0.0f, -0.1f, 0.0f), new Vector3i(0, 1, 0));
        ...
        //    this.getPhysicsGhostObject().boundingBox(boundingBox);
        //    RayResult result = RayResult.getRayHitResult(this.getWorld().getDynamics(), physicsGhostObject, sweepResult.getCorrectedPos(), new Vector3f(sweepResult.getCorrectedPos()).sub(0.0f, boundingBox.getYExtent() * 2.0f + this.getStepHeight(), 0.0f));
    ...
    private boolean checkAngleOnPos(PhysicsGhostObject physicsGhostObject, double toCheckAngle, Vector3f pos, Vector3f vector, boolean ifNoHit) {
    ...
    private float getStepDownY() {
        ...
        SweepResult sweepResult = SweepResult.getSweepHitResult(this.getWorld().getDynamics(), this.getPhysicsGhostObject(), (ConvexShape) this.getPhysicsGhostObject().getCollisionShape(), this.getPosition(), new Vector3f(0.0f, -maxY, 0.0f), new Vector3i(0, 1, 0));
        if (sweepResult.getHitNormal() != null) {
    ...
    private Vector3f tryStepUp(Vector3f currPos, Vector3f motion, float height) {
        ...
        SweepResult sweepResultStep = SweepResult.getSweepHitResult(this.getWorld().getDynamics(), this.getPhysicsGhostObject(), (ConvexShape) this.getPhysicsGhostObject().getCollisionShape(), getHitUp, motion, new Vector3i(1));
        Vector3f getHitStep = sweepResultStep.getCorrectedPos();

        BoundingBox boundingBox = new BoundingBox();
        this.getPhysicsGhostObject().boundingBox(boundingBox);

        //RayResult result = RayResult.getRayHitResult(this.getWorld().getDynamics(), this.getPhysicsGhostObject(), getHitStep, new Vector3f(getHitStep).sub(0.0f, boundingBox.getYExtent() * 2.0f + height, 0.0f));
        //if (result.getHitNormal() != null) {
    ...
    private float checkSlopeY(Vector3f checkFrom, Vector3f motion) {
        SweepResult sweepResult0 = SweepResult.getSweepHitResult(this.getWorld().getDynamics(), this.getPhysicsGhostObject(), (ConvexShape) this.getPhysicsGhostObject().getCollisionShape(), checkFrom, motion, new Vector3i(1));
    ...
    private float getLiquidSwimUpFactor() {
        ...
        this.getWorld().getDynamics().getPhysicsSpace().contactTest(this.getPhysicsGhostObject(), (e) -> {
    ...
    public boolean moveTestHorizontalXZ(Vector3f motion, final float ySlope) {
        ...
        SweepResult slideResult2 = SweepResult.getSweepHitResult(this.getWorld().getDynamics(), this.getPhysicsGhostObject(), (ConvexShape) this.getPhysicsGhostObject().getCollisionShape(), new Vector3f(corrected), new Vector3f(slide), new Vector3i(1, 0, 1));
        if (slide.length() > 0.0f && slideResult2.getHitNormal() != null) {
            ...
            if (slideResult2.getSlideMotion().y > 0.0f) {
                if (this.checkAngleOnPos(this.getPhysicsGhostObject(), this.getSlopeAngle(), new Vector3f(corrPosSlide).add(0.0f, slideResult2.getSlideMotion().y, 0.0f), new Vector3f(0.0f, -0.1f, 0.0f), false)) {
    ...
    public int getCollisionGroup() {
        return this.getPhysicsGhostObject().getCollisionGroup();
    }
    ...
    public void setCollisionGroup(CollisionFilter... collisionFilters) {
        ...
        this.getPhysicsGhostObject().setCollisionGroup(i);
    }

    public int getCollisionFilter() {
        return this.getPhysicsGhostObject().getCollideWithGroups();
    }
    ...
    public void setCollisionFilter(CollisionFilter... collisionFilters) {
        ...
        this.getPhysicsGhostObject().setCollideWithGroups(i);
    }
    ...
    private static class RayResult {
        ...
        public static JGemsKinematicItem.RayResult getRayHitResult(DynamicsSystem dynamicsSystem, PhysicsGhostObject ghostObject, Vector3f posFrom, Vector3f posTo) {
    ...
    private static class SweepResult {
        ...
        public static SweepResult getSweepHitResult(DynamicsSystem dynamicsSystem, PhysicsGhostObject ghostObject, ConvexShape convexShape, Vector3f posFrom, Vector3f motion, Vector3i axis, float ccd) {
