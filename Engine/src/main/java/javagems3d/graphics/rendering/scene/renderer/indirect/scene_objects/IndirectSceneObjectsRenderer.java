...
public abstract class IndirectSceneObjectsRenderer {
    protected static final int SSBO_DATASETS_MATRICES_SIZE = JGemsConfig.SYSTEM.MAX_INDIRECT_SCENE_OBJ_RENDERING_MESH_DATASETS * 16;
    protected static final int SSBO_DATASETS_ENT_IDS_SIZE = JGemsConfig.SYSTEM.MAX_INDIRECT_SCENE_OBJ_RENDERING_MESH_DATASETS;
    protected static final int SSBO_DATASETS_MATERIAL_IDS_SIZE = JGemsConfig.SYSTEM.MAX_INDIRECT_SCENE_OBJ_RENDERING_MESH_MATERIALS;
    protected static final int SSBO_DATASETS_PROPERTIES_SIZE = JGemsConfig.SYSTEM.INDIRECT_SCENE_OBJ_RENDERING_PROPERTIES_PACK_SIZE * JGemsConfig.SYSTEM.MAX_INDIRECT_SCENE_OBJ_RENDERING_MESH_PROPERTIES;
    ...
    public IndirectSceneObjectsRenderer(@NotNull OpenGLRenderer openGLRenderer, @NotNull ShaderStorageBufferObject indirectSSBO, @NotNull ShaderStorageBufferObject propertiesSSBO, @NotNull Pipeline pipeline, boolean usePropertiesSSBO, boolean useMaterialsSSBO) {
    ...
    protected void fillSSBOWithSceneObjectInformation(@NotNull IntBuffer indexes, @NotNull IntBuffer materialIds, Collection<SceneObject> sceneObjects) {
        ByteBuffer properties = this.isUsePropertiesSSBO() ? MemoryUtil.memAlloc(Float.BYTES * IndirectSceneObjectsRenderer.SSBO_DATASETS_PROPERTIES_SIZE) : null;
        FloatBuffer modelMatrices = MemoryUtil.memAllocFloat(IndirectSceneObjectsRenderer.SSBO_DATASETS_MATRICES_SIZE);
        FloatBuffer deltaFrames = MemoryUtil.memAllocFloat(IndirectSceneObjectsRenderer.SSBO_DATASETS_ENT_IDS_SIZE);
    ...
    public record Operator(IRenderingFunction renderingFunction, JGemsShaderManager indirectShader) {
            public Operator(@NotNull IndirectSceneObjectsRenderer.IRenderingFunction renderingFunction, @NotNull JGemsShaderManager indirectShader) {
