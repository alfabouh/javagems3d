...
public class Texture2DMSAAProgram implements ITexture2DProgram, ITextureBindless {
    ...
    public void createTexture(Vector2i size, @NotNull Texture2DMSAAProgram.Properties properties, FloatBuffer pixels) {
        ...
        this.bindTexture();
        this.size = size;
        GL46.glTexImage2DMultisample(this.getTextureAttachment(), this.msaa, properties.internalFormat(), size.x, size.y, true);
        this.unBindTexture();
    ...
    public record Properties(int internalFormat, int filteringMag, int filteringMin, int compareMode, int compareFunc,
                             int clampS, int clampT, float[] borderColor) implements IProperties {
    }
}