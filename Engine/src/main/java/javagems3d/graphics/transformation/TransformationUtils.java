...
public abstract class TransformationUtils {
    ...
    public static Matrix4f getModelViewMatrix(Format3D format3D, Matrix4f viewMatrix) {
        if (format3D.isOrientedToViewMatrix()) {
            return TransformationUtils.getOrientedToViewModelViewMatrix(format3D, viewMatrix);
        }
        return new Matrix4f(viewMatrix).mul(TransformationUtils.getModelMatrix(format3D));
    }

    public static Matrix4f getOrientedToViewModelViewMatrix(Format3D format3D, Matrix4f viewMatrix) {
        Matrix4f m1 = TransformationUtils.getModelMatrix(format3D);
    ...
    public static List<Matrix4f> getAllDirectionViewSpaces(Vector3f pos, float near, float far) {
        ...
        Matrix4f projectionViewMatrix1 = new Matrix4f(perspective).mul(TransformationUtils.getLookAtMatrix(pos, new Vector3f(0.0f, -1.0f, 0.0f), new Vector3f(pos).add(1.0f, 0.0f, 0.0f)));
        Matrix4f projectionViewMatrix2 = new Matrix4f(perspective).mul(TransformationUtils.getLookAtMatrix(pos, new Vector3f(0.0f, -1.0f, 0.0f), new Vector3f(pos).add(-1.0f, 0.0f, 0.0f)));
        Matrix4f projectionViewMatrix3 = new Matrix4f(perspective).mul(TransformationUtils.getLookAtMatrix(pos, new Vector3f(0.0f, 0.0f, 1.0f), new Vector3f(pos).add(0.0f, 1.0f, 0.0f)));
        Matrix4f projectionViewMatrix4 = new Matrix4f(perspective).mul(TransformationUtils.getLookAtMatrix(pos, new Vector3f(0.0f, 0.0f, -1.0f), new Vector3f(pos).add(0.0f, -1.0f, 0.0f)));
        Matrix4f projectionViewMatrix5 = new Matrix4f(perspective).mul(TransformationUtils.getLookAtMatrix(pos, new Vector3f(0.0f, -1.0f, 0.0f), new Vector3f(pos).add(0.0f, 0.0f, 1.0f)));
        Matrix4f projectionViewMatrix6 = new Matrix4f(perspective).mul(TransformationUtils.getLookAtMatrix(pos, new Vector3f(0.0f, -1.0f, 0.0f), new Vector3f(pos).add(0.0f, 0.0f, -1.0f)));
