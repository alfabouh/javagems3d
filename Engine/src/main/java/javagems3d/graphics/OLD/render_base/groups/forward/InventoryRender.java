...
public class InventoryRender extends SceneRenderBase {
    public InventoryRender(JGemsOpenGLRendererOLD sceneRender) {
        super(-1, sceneRender, new RenderGroup("INVENTORY_FORWARD"));
    }

    public void onRender(FrameTicking frameTicking) {
        IPlayer player = JGems3D.get().getPlayer();
        if (player instanceof IInventoryOwner && !(JGemsHelper.CAMERA.getCurrentCamera() instanceof ControlledCamera)) {
