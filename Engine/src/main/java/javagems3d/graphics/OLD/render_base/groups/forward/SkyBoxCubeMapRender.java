...
public class SkyBoxCubeMapRender extends SceneRenderBase {
    ...
    public SkyBoxCubeMapRender(SkyBox skyBox, JGemsOpenGLRendererOLD sceneRender) {
    ...
    private void renderSkyBoxTexture() {
        ...
        shaderManager.performUniform(new UniformString("covered_by_fog"), UniformFunctions.BOOLEAN(this.getSkyBox().isSkyCoveredByFog()));
        shaderManager.performUniform(new UniformString("view_mat_inverted"), UniformFunctions.MAT4F(new Matrix4f(JGemsSceneUtils.getMainCameraViewMatrix()).invert()));
