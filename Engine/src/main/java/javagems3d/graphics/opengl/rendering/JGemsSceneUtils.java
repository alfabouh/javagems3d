...
public abstract class JGemsSceneUtils {
    ...
    public static void renderModeledSceneObject(AbstractSceneObject sceneObject, ICamera camera) {
        if (sceneObject != null) {
            ...
            JGemsShaderManager shaderManager = sceneObject.getObjectRenderSettings().getModelRenderShader();
            ...
            if (sceneObject.getObjectRenderSettings().isDisabledFaceCulling()) {
            ...
            for (MeshGroup.MeshGroupNode meshNode : model.<MeshGroup>getMeshStructureWithUnSafeCast().getMeshNodes()) {
                Material material = meshNode.getMaterial();
                if (sceneObject.getObjectRenderSettings().isAllowedMoveMeshesIntoTransparencyPass()) {
                    if (material.hasTransparency()) {
                        gemsOpenGLRenderer.addModelNodeInTransparencyPass(new WorldTransparentRender.RenderNodeInfo(null, sceneObject.getObjectRenderSettings().isDisabledFaceCulling(), meshNode, model.getFormat()));
                        continue;
                    }
                }
    ...
    private static boolean filterObjectTransparency(AbstractSceneObject sceneObject) {
        if (sceneObject.getObjectRenderSettings().isAllowedMoveMeshesIntoTransparencyPass()) {
            if (sceneObject.getObjectRenderSettings().getModelRenderShader().checkShaderRenderPass(RenderPass.TRANSPARENCY)) {
                return true;
            }
        }
        return false;
