...
public class DefaultDirectSimpleRenderFabric extends DirectRenderFabric {
    public DefaultDirectSimpleRenderFabric(Stage stage) {
        super(stage);
    }
    ...
    public void onRender(Pipeline pipeline, JGemsShaderManager shaderManager, OpenGLRenderer openGLRenderer, IRendered renderedItem, ArbitraryArguments metaData) {
        if (renderedItem instanceof WBenchMarkerObject) {
            ...
            if (renderedItem.canBeRendered()) {
                ...
                shaderManager.performUniform(new UniformString("color"), UniformFunctions.VEC3F(markerObject.getColor()));
