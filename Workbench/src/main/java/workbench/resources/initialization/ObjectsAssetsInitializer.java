...
public class ObjectsAssetsInitializer implements IAssetsInitializer {
    public void load(SystemResources systemResources) {
...

        {
            ...
            for (Map.Entry<String, ResourceProp> entry : propEntry) {
                ResourceProp resourceEntity = entry.getValue();
                WBenchObjectData wBenchObjectData = resourceEntity.getFabricWBench().create();
                WBenchObjectTemplate wBenchObjectTemplate = this.constructObjectTemplate(systemResources, resourceEntity.getId(), wBenchObjectData);
                WBench.get().getProjectObjects().addProp(resourceEntity.getGroupId(), wBenchObjectTemplate);
            }
        }

        {
            ...
            for (Map.Entry<String, ResourceMarker> entry : markerEntry) {
                ResourceMarker resourceEntity = entry.getValue();
                WBenchMarkerData wBenchObjectData = resourceEntity.getFabricWBench().create();
                WBenchMarkerTemplate wBenchMarkerTemplate = this.constructMarkerTemplate(systemResources, resourceEntity.getId(), wBenchObjectData);
                WBench.get().getProjectObjects().addMarker(resourceEntity.getGroupId(), wBenchMarkerTemplate);
            }
        }
    }
    ...
    private WBenchMarkerTemplate constructMarkerTemplate(SystemResources systemResources, String id, WBenchMarkerData wBenchMarkerData) {
        ...
        return new WBenchMarkerTemplate(id, meshGroup, wBenchMarkerData.getTagsContainer(), wBenchMarkerData.getTranslationConstraints(), wBenchMarkerData.getColor(), wBenchMarkerData.isTransparent());
    }
