...
public class WBenchMapEditorTextureAssetsInitializer implements IAssetsInitializer {
    public void load(SystemResources systemResources) {
        ...
        for (Map.Entry<String, Pair<String, JGemsPath>> entry : map.entrySet()) {
            ICubeMapProgram cubeMapProgram = systemResources.createCubeMapTexture(JGems3D.GetSource.JAR, null, entry.getValue().getSecond(), entry.getValue().getFirst(), new CubeMapTexture.Properties(true));
